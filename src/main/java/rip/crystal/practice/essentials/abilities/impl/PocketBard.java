    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (profile.getPocketbard().onCooldown(player)) {
                player.sendMessage(CC.translate("&7You are on &4&lPocket Bard &4cooldown for &4" + DurationFormatter.getRemaining(profile.getPocketbard().getRemainingMilis(player), true, true)));
