...
public class KitsCommand extends BaseCommand {
	...
	public void onCommand(CommandArgs commandArgs) {
		...
		for (Kit kit : Kit.getKits()) {
			ChatComponentBuilder builder = new ChatComponentBuilder("").parse("&7- " + (kit.isEnabled() ? "&8[&a&lEnabled&8] &c&l" : "&8[&c&lDisabled&8] &c&l") + kit.getName() + " &7(" + (kit.getGameRules().isRanked() ? "Ranked" : "Un-Ranked") + ")");
