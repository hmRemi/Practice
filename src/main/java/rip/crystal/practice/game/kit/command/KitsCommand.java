...
public class KitsCommand extends BaseCommand {
	...
	public void onCommand(CommandArgs commandArgs) {
		...
		for (Kit kit : Kit.getKits()) {
			ChatComponentBuilder builder = new ChatComponentBuilder("").parse("&7- " + (kit.isEnabled() ? "&8[&a&lEnabled&8] &4&l" : "&8[&4&lDisabled&8] &4&l") + kit.getName() + " &7(" + (kit.getGameRules().isRanked() ? "Ranked" : "Un-Ranked") + ")");
