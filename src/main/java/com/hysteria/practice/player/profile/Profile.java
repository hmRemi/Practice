...
public class Profile {
	...
	private Lang locale = Lang.getByAbbreviation(HyPractice.get().getMainConfig().getString("DEFAULT_LANG"));
	...
	public static void init() {
		if (HyPractice.get().getRankManager().getRank() instanceof Default) {
			for (int i = 0; i < 3; i++) System.out.println("HyPractice needs a permission system to function properly");
			Bukkit.getServer().getPluginManager().disablePlugin(HyPractice.get());
		}

		// Players might have joined before the plugin finished loading
		TaskUtil.runLater(() -> {
			if (iProfile instanceof MongoDBIProfile) {
				collection = HyPractice.get().getMongoConnection().getMongoDatabase().getCollection("profiles");
				for (Document document : HyPractice.get().getMongoConnection().getMongoDatabase().getCollection("profiles").find()) {
				...
				for (String players : HyPractice.get().getPlayersConfig().getConfiguration().getConfigurationSection("players").getKeys(false)) {
	...
	public void follow(Player player) {
		this.follow = new Follow(this.uuid, player);
		Follow.getFollows().put(this.uuid, this.follow);
		this.follow.follow();
	}

	public void unfollow(Player player) {
		Follow.getFollows().remove(this.uuid, this.follow);
	}
