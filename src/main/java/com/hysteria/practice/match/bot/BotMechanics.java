...
public class BotMechanics extends BukkitRunnable
{
    ...
    private void startupBotMechanics() {
        ...
        if (this.difficulty == com.hysteria.practice.match.bot.Bot.BotDifficulty.EASY) {
            this.attackRange *= 0.8;
            this.swingRangeModifier = -0.5;
        }
        else if (this.difficulty == com.hysteria.practice.match.bot.Bot.BotDifficulty.HARD) {
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            if (BotMechanics.this.Bot.getBukkitEntity() != null) {
                                ...
                                }.runTaskLater((Plugin) HyPractice.get(), (long)(BotMechanics.this.random.nextInt(4) + 2));
                            }
                        }
                    }.runTaskLater((Plugin) HyPractice.get(), 35L);
                }
            }.runTaskLater((Plugin) HyPractice.get(), (long)(this.random.nextInt(2) + 1));
            return true;
        }
        return false;
    }
            ...
            new BukkitRunnable() {
                public void run() {
                    ...
                    if (!BotMechanics.this.Bot.getBukkitEntity().isDead()) {
                        ...
                        }.runTaskLater((Plugin) HyPractice.get(), (long)(BotMechanics.this.random.nextInt(1) + 1));
                    }
                }
            }.runTaskLater((Plugin) HyPractice.get(), (long)(this.random.nextInt(1) + 1));
            return true;
        }
        return false;
    }
                ...
                public void run() {
                    if (BotMechanics.this.Bot.getNpc() != null && BotMechanics.this.Bot.isSpawned() && BotMechanics.this.Bot.getNpc().getNavigator() != null) {
                        ...
                        if (this.counter == 0 || Math.abs(BotMechanics.this.Bot.getBukkitEntity().getLocation().getPitch() - 90.0f) < 50.0f) {
                            ...
                            }.runTaskTimer((Plugin) HyPractice.get(), 1L, 1L);
                            BotMechanics.this.Bot.getBukkitEntity().setItemInHand(new ItemStack(Material.AIR));
                            ...
                            }.runTaskLater((Plugin) HyPractice.get(), (long)(BotMechanics.this.random.nextInt(12) + 8));
                        }
                    }
                    else {
                        this.cancel();
