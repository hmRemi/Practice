package com.hysteria.practice.match.listeners;
...
public class MatchListener implements Listener {
	...
	public void onPlayerDeathEvent(PlayerDeathEvent event) {
		...
		if (profile.getState() == ProfileState.FIGHTING) {
			...
			if (match.getKit().getGameRules().isBridge() || match.getKit().getGameRules().isBedFight() || match.getKit().getGameRules().isLives()) {
			...
			if(match.getKit().getGameRules().isLives()) {
				if(match.canEndMatch()) match.setState(MatchState.ENDING_MATCH);
			}

			if (HyPractice.get().getMainConfig().getBoolean("MATCH.DROP_ITEMS_ON_DEATH")) {
				TaskUtil.run(() -> {
			...
			if (profile.getState() == ProfileState.FIGHTING) {
				...
				if (match.getState() == MatchState.STARTING_ROUND) event.setCancelled(true);
					...
					if (event.getEntity() instanceof ThrownPotion) {
						if (HyPractice.get().getMainConfig().getBoolean("MATCH.FAST_POTION")) {
		...
		if (attacker != null && event.getEntity() instanceof Player) {
			...
			if (damagedProfile.getState() == ProfileState.FIGHTING && attackerProfile.getState() == ProfileState.FIGHTING) {
				...
				if (match.getKit().getGameRules().isBoxing()) {
					if (match.getGamePlayer(attacker).getHits() == HyPractice.get().getMainConfig().getInteger("MATCH.BOXING_MAX_HITS")) {
...
//					TaskUtil.runLater(() -> victim.removeMetadata("lastAttacker", HyPractice.get()), 20L * 15);
//				}
			}
		}
	}
...
//				}
	...
	public void onMatchEnd(MatchEndEvent event) {
		...
		match.getParticipants().forEach(matchGamePlayerGameParticipant -> matchGamePlayerGameParticipant.getPlayers().forEach(player -> {
