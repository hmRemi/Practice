...
public class MatchTask extends BukkitRunnable {
    ...
    public void run() {
        if (Arrays.asList(30, 15, 10, 5, 4, 3, 2, 1).contains(seconds)) {
            tntTagGameLogic.getParticipants().forEach(gamePlayerGameParticipant -> new MessageFormat(Locale.EVENT_MATCH_REMAINING.format(Profile.get(gamePlayerGameParticipant.getLeader().getUuid()).getLocale()))
