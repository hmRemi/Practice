...
public class TNTTagGameLogic implements EventGameLogic {
        ...
        if (winningParticipant != null) {
            Bukkit.getOnlinePlayers().forEach(player -> {
                ...
                new MessageFormat(Locale.EVENT_FINISH.format(profile.getLocale()))
                    .add("{event_name}", game.getEvent().getName())
                    .add("{event_displayname}", game.getEvent().getDisplayName())
                    .add("{winner}", HyPractice.get().getRankManager().getRank().getPrefix(winningParticipant.getLeader().getUuid()) + winningParticipant.getConjoinedNames())
    ...
    public void startRound() {
        ...
        int seconds = HyPractice.get().getEventsConfig().getInteger("EVENTS.TNTTAG.MATCH-SECONDS");
    ...
    public void onDeath(Player player, Player killer) {
        ...
        getParticipants().forEach(gamePlayerGameParticipant -> gamePlayerGameParticipant.getPlayers().forEach(gamePlayer -> {
    ...
    public List<String> getScoreboardEntries() {
        ...
        config.getStringList("EVENTS.TNTTAG.LINES").forEach(s -> lines.add(s.replace("{event-name}", game.getEvent().getName())
        ...
        switch (game.getGameState()) {
            ...
            break;
            case STARTING_EVENT: {
                config.getStringList("EVENTS.TNTTAG.STARTING-EVENT").forEach(s -> lines.add(s.replace("{time}", String.valueOf(game.getGameLogic().getGameLogicTask().getNextActionTime()))
                        .replace("{bars}", CC.SB_BAR)));
            }
            break;
            case PLAYING_ROUND: {
                config.getStringList("EVENTS.TNTTAG.PLAYING-ROUND").forEach(s -> lines.add(s.replace("{bomb}", getBomb().getConjoinedNames())
            ...
            case STARTING_ROUND:
            case ENDING_ROUND: {
                config.getStringList("EVENTS.TNTTAG.ENDING-ROUND").forEach(s -> lines.add(s.replace("{bars}", CC.SB_BAR)));
            }
            break;
            case ENDING_EVENT: {
                if (winningParticipant != null) {
                    config.getStringList("EVENTS.TNTTAG.ENDING-EVENT").forEach(s -> lines.add(s.replace("{bars}", CC.SB_BAR)
        ...
        if (game.getGameState() == EventGameState.WAITING_FOR_PLAYERS ||
            ...
            config.getStringList("EVENTS.TNTTAG.MAP-VOTES").forEach(s -> {
                if (s.contains("{votes-format}")) {
                    game.getVotesData().forEach((map, voteData) -> lines.add(config.getString("EVENTS.TNTTAG.VOTES-FORMAT")
    ...
    private void setBombDress(GameParticipant<GamePlayer> target) {
        ...
        getParticipants().forEach(gamePlayerGameParticipant -> new MessageFormat(Locale.EVENT_ITS_THE_BOMB.format(Profile.get(gamePlayerGameParticipant.getLeader().getUuid()).getLocale()))
