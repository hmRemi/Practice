...
//        participants.remove(getGameParticipant(player));
    ...
    public List<String> getScoreboardEntries() {
        ...
        config.getStringList("EVENTS.TNTRUN.LINES").forEach(s -> lines.add(s.replace("{event-name}", game.getEvent().getName())
        ...
        switch (game.getGameState()) {
            ...
            break;
            case STARTING_EVENT: {
                config.getStringList("EVENTS.TNTRUN.STARTING-EVENT").forEach(s -> lines.add(s.replace("{time}", String.valueOf(game.getGameLogic().getGameLogicTask().getNextActionTime()))
                        .replace("{bars}", CC.SB_BAR)));
            }
            break;
            case STARTING_ROUND:
            case ENDING_ROUND: {
                config.getStringList("EVENTS.TNTRUN.ENDING-ROUND").forEach(s -> lines.add(s.replace("{bars}", CC.SB_BAR)));
            }
            break;
            case ENDING_EVENT: {
                if (winningParticipant != null) {
                    config.getStringList("EVENTS.TNTRUN.ENDING-EVENT").forEach(s -> lines.add(s.replace("{bars}", CC.SB_BAR)
        ...
        if (game.getGameState() == EventGameState.WAITING_FOR_PLAYERS ||
            ...
            config.getStringList("EVENTS.TNTRUN.MAP-VOTES").forEach(s -> {
                if (s.contains("{votes-format}")) {
                    game.getVotesData().forEach((map, voteData) -> lines.add(config.getString("EVENTS.TNTRUN.VOTES-FORMAT")
