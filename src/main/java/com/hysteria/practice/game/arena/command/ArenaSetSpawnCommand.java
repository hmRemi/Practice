...
public class ArenaSetSpawnCommand extends BaseCommand {
	...
	public void onCommand(CommandArgs commandArgs) {
		...
		if (args[0].equalsIgnoreCase("safezone")) {
			...
			HyPractice.get().getFfaManager().setFfaSafezone(new Cuboid(selection.getPoint1(), selection.getPoint2()));
			FileConfiguration configuration = HyPractice.get().getMainConfig().getConfiguration();
			configuration.set("FFA.SAFEZONE.location1", LocationUtil.serialize(HyPractice.get().getFfaManager().getFfaSafezone().getLowerCorner()));
			configuration.set("FFA.SAFEZONE.location2", LocationUtil.serialize(HyPractice.get().getFfaManager().getFfaSafezone().getUpperCorner()));
			player.sendMessage(CC.translate("&aYou have set the FFA Safezone."));
			try {
				configuration.save(HyPractice.get().getMainConfig().getFile());
			} catch (IOException e) {
				e.printStackTrace();
			}
