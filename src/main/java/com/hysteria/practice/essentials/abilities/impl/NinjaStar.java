    ...
    private void onInteract(PlayerInteractEvent event) {
        ...
        if (event.getAction().equals(Action.RIGHT_CLICK_AIR) || event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
            ...
            if (profile.getNinjastar().onCooldown(player)) {
                player.sendMessage(CC.translate("&7You are on &c&lNinja Star &7cooldown for &4" + DurationFormatter.getRemaining(profile.getNinjastar().getRemainingMillis(player), true, true)));
