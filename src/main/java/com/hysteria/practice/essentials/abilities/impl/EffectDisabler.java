    ...
    private void onDamage(EntityDamageByEntityEvent event) {
        if (event.getEntity() instanceof Player && event.getDamager() instanceof Player) {
            ...
            if (profile.getEffectdisabler().onCooldown(damager)) {
                damager.sendMessage(CC.translate("&7You are on &c&lEffect Disabler &7cooldown for &4" + DurationFormatter.getRemaining(profile.getEffectdisabler().getRemainingMilis(damager), true, true)));
