...
public class CosmeticsShopButton extends Button {
    ...
    public ItemStack getButtonItem(Player player) {
        ...
        for (String text : HyPractice.get().getMenuConfig().getStringList("COIN-SHOP.COSMETICS.LORE")) {
            lore.add(text.replace("{coins}", profile.getCoins() + ""));
        }

        return new ItemBuilder(Material.IRON_CHESTPLATE)
                .name(HyPractice.get().getMenuConfig().getString("COIN-SHOP.COSMETICS.ITEM-NAME"))
                .lore(lore)
                .build();
    }
