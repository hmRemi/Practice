...
public class InventoryUtil {
	...
	public static void saveItemStacksKit(String path, ItemStack[] source, boolean armor) {
		for (int b = 0; b < source.length; b++) {
			if (armor) {
				HyPractice.get().getKitsConfig().getConfiguration().set(path + ".loadout.armor." + b, Arrays.asList(source).get(b));
			} else {
				HyPractice.get().getKitsConfig().getConfiguration().set(path + ".loadout.contents." + b, Arrays.asList(source).get(b));
			}
		}
	}
	...
	public static ItemStack[] serializeItemStackKit(String source) {
		...
		HyPractice.get().getKitsConfig().getConfiguration().getConfigurationSection(source).getKeys(false)
				.forEach(s ->
					items.add(HyPractice.get().getKitsConfig().getConfiguration().getItemStack(source + "." + s)));
...
//	}
	...
	public static void removeCrafting(Material material) {
		Iterator<Recipe> iterator = HyPractice.get().getServer().recipeIterator();
